...
public class OrderQueryTool implements Function<OrderQueryTool.Request, OrderQueryTool.Response> {
    ...
    public OrderQueryTool(RestClient mallRestClient) {
    ...
    public Response apply(Request request) {
        ...
        if (!context.isLoggedIn() || !context.hasUserId() || !context.matchesScenario(ALLOWED_SCENARIOS)) {
            log.warn("订单工具权限拒绝, userId={}, loginStatus={}, scenario={}, orderId={}",
                    maskUserId(context.userId()), context.loginStatus(), context.scenario(),
                    request == null ? "" : request.orderId());
            return new Response("当前身份或场景无权查询订单，请先登录后重试");
        }
        ...
        try {
            ...
            log.error("查询订单失败, userId={}, scenario={}, orderId={}",
                    maskUserId(context.userId()), context.scenario(), request.orderId(), e);
            return new Response("查询订单失败，请稍后重试");
        }
    }
